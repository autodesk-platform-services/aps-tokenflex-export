  ...
  <v-content>
    <v-dialog v-model="dialog" max-width="80%">
      <v-card v-bind:style="(dialogChartOpts || {}).styleObject">
        <v-card-title v-if="!dialogChartOpts" class="grey lighten-3">{{
          dialogHeader
        }}</v-card-title>
        ...
        <v-container v-if="dialogChartOpts">
          <Chart
            :datasets="dialogChartOpts.datasets"
            :type="dialogChartOpts.type"
            :labels="dialogChartOpts.labels"
            :title="dialogChartOpts.title"
            :styleObject="dialogChartOpts.chartStyleObject"
          />
          <br />
        </v-container>
        ...
        <gmap-map
          v-if="dialogGmapOpts"
          :center="dialogGmapOpts.markers[0].position"
          :zoom="zoom"
        ...
        >
          ...
          <gmap-marker
            :key="index"
            v-for="(m, index) in dialogGmapOpts.markers"
            :title="m.title"
            :position="m.position"
            :clickable="true"
    ...
    <v-container fluid grid-list-md>
      <v-card>
        <v-toolbar color="white" flat>
          ...
          >
          <v-spacer></v-spacer>
          <v-btn :disabled="!!!items.length" @click="exportContracts"
            >Export</v-btn
          >
          <v-btn icon light>
        ...
        <v-progress-linear
          :indeterminate="true"
          v-if="this.$store.state.loading.contracts"
        ></v-progress-linear>

        <v-container
          fluid
          grid-list-md
          v-if="!this.$store.state.loading.contracts && showContract"
          transition="slide-y-transition"
        >
          <v-layout row wrap>
            <v-flex xs4>
              <v-card hover class="pa-1">
                <h3 class="text-xs-center">Contract Count</h3>
                <h1 class="text-xs-center">
                  <animated-number :value="contractCount" :duration="300" />
                </h1>
              </v-card>
            </v-flex>
            <v-flex xs4>
              <v-card hover class="pa-1">
                <h3 class="text-xs-center">Length in Days</h3>
                <h1 class="text-xs-center">
                  <animated-number :value="contractLength" :duration="300" />
                </h1>
              </v-card>
            </v-flex>
            <v-flex xs4>
              <v-card hover class="pa-1">
                <h3 class="text-xs-center">Average Days</h3>
                <h1 class="text-xs-center">
                  <animated-number :value="averageLength" :duration="300" />
                </h1>
              </v-card>
            </v-flex>
          </v-layout>
          ...
          >
            <v-flex slot="item" slot-scope="props" xs12 sm6 md4 lg3>
              <v-card
                ...
                hover
                v-on:click.native="
                  showContractStats(props.item.contractNumber, true)
                "
              >
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn @click.stop="displayMap(props.item, props.index)">{{
                    props.item.showMap || props.item.showLoading
                      ? "info"
                      : "map view"
                  }}</v-btn>
                  <v-btn
                    @click.stop="showDialogMap(props.item)"
                    v-if="props.item.showMap && !props.item.showLoading"
                    >enlarge</v-btn
                  >
                </v-card-actions>
                <v-divider></v-divider>
                <v-list v-if="!props.item.showMap">
                  ...
                  <v-list-tile>
                    <v-list-tile-content
                      >Contract Start Date:</v-list-tile-content
                    >
                    <v-list-tile-content class="align-end">{{
                      props.item.contractStartDate
                    }}</v-list-tile-content>
                  </v-list-tile>
                  <v-list-tile>
                    <v-list-tile-content
                      >Contract End Date:</v-list-tile-content
                    >
                    <v-list-tile-content class="align-end">{{
                      props.item.contractEndDate
                    }}</v-list-tile-content>
                  </v-list-tile>
                  <v-list-tile>
                ...
                <div @click.stop="">
                  <gmap-map
                    :center="props.item.markers[0].position"
                    :zoom="zoom"
                    style="width: 100%; height: 255px"
                    :mapTypeControl="false"
                    v-if="props.item.showMap"
                  >
                    <gmap-info-window
                      :options="infoOptions"
                      :position="props.item.infoWindowPos"
                      :opened="props.item.infoWinOpen"
                      @closeclick="props.item.infoWinOpen = false"
                    >
                      <div v-html="props.item.infoContent"></div>
                    </gmap-info-window>
                    <gmap-marker
                      :key="index"
                      v-for="(m, index) in props.item.markers"
                      :title="m.title"
                      :position="m.position"
                      :clickable="true"
                      @click="toggleInfoWindow(m, index, props.item)"
                    ></gmap-marker>
                  </gmap-map>
                </div>
                <v-progress-linear
                  :indeterminate="true"
                  v-if="props.item.showLoading"
                ></v-progress-linear>
              </v-card>
            </v-flex>
          </v-data-iterator>
      ...
      <v-card>
        <v-toolbar color="white" flat>
          <v-toolbar-title class="grey--text text--darken-4"
            >Statistics
            {{
              !$vuetify.breakpoint.xs && $store.state.contractNumber
                ? `- ${$store.state.contractNumber}`
                : ""
            }}</v-toolbar-title
          >
          <v-spacer></v-spacer>
        ...
        <v-progress-linear
          :indeterminate="true"
          v-if="this.$store.state.loading.charts"
        ></v-progress-linear>

        <v-container
          fluid
          grid-list-md
          v-if="!this.$store.state.loading.charts && showCharts"
        >
          <v-layout
            row
            wrap
            v-for="(ucr, index) in usageChartOpts"
            :key="index"
          >
            <v-flex
              v-bind="{
                [$vuetify.breakpoint.xs
                  ...
                  ? `xs${Math.round(12 / ucr.length) * 2}`
                  : `xs${Math.round(12 / ucr.length)}`]: true,
              }"
              v-for="(uc, index) in ucr"
              :key="index"
            >
              <v-card
                raised
                hover
                v-on:click.native="showDialogChart(uc)"
                v-bind:style="uc.styleObject"
              >
                <Chart
                  ...
                  :labels="uc.labels"
                  :title="uc.title"
                  :styleObject="uc.chartStyleObject"
                />
                <br />
              </v-card>
            </v-flex>
...
<style>
.vue-map-container .gm-svpc,
.vue-map-container .gm-style-mtc {
...
export default {
  beforeMount() {
    if (this.$store.state.isUserLoggedIn) this.getContracts();
  },
  components: {
    Chart,
    AnimatedNumber,
  ...
  computed: {
    ...
    contractLength: function () {
      return this.items.reduce(
        (o, e) => diffDays(e.contractStartDate, e.contractEndDate) + o,
        0
      );
    },
    averageLength: function () {
      return this.contractLength / this.items.length;
    },
  },
  methods: {
    showDialogMap(item) {
    ...
    async displayMap(item, index) {
      if (item.showLoading || item.showMap) {
        if (item.cancelSource) {
          item.cancelSource.cancel("Operation canceled due to new requests");
        }
      ...
      await this.$axios({
        ...
        url: new URL(
          `/api/aps/reports/${item.contractNumber}/query`,
          config.koahost
        ).href,
      })
        .then((response) => {
          ...
          if (response.data) {
            ...
            let ris = [];
            if (rows.length > 50) {
              // randomly pick 5-10 geos if greater than 50 in total
              ...
              Object.assign(item, {
                ...
                markers: ris.map((ri) => {
                  ...
                  return {
                    ...
                    infoText: `Product: ${row[2]}<br/>Tokens Consumed: ${
                      row[3]
                    }<br/>Usage: ${Math.round(row[4])} hours`,
                  };
                }),
              })
            );
            setTimeout(() =>
              children
                .find((child) => child.$refs["vue-map"])
                .$mapPromise.then(() =>
                  this.$set(
                    this.items,
                    index,
                    Object.assign(item, { showLoading: false })
                  )
                )
            );
          }
        ...
        .catch((err) => {
          this.showError(err);
          console.error(
            `/api/aps/reports/${item.contractNumber}/query error`,
            err
          );
        });
    },
    async getContracts() {
      ...
      await this.$axios({
        method: "GET",
        url: new URL("/api/aps/reports/contracts", config.koahost).href,
      })
        .then((response) => {
          if (response.data) {
        ...
        .catch((err) => {
          ...
          console.error(`/api/aps/reports/contracts error`, err);
        })
        .finally(() =>
          this.$store.dispatch("setLoading", { contracts: false })
        );
    },
    async showContractStats(contractNumber, doScrollTo) {
      if (doScrollTo)
        window.scroll({
          top: this.$refs["statsHolder"].offsetTop - document.body.scrollTop,
          left: 0,
          behavior: "smooth",
        });
      if (this.showStatsCancelSource) {
        this.showStatsCancelSource.cancel(
          "Operation canceled due to new requests"
        );
      ...
      this.$axios({
        method: "GET",
        url: new URL(
          `/api/aps/reports/summary/${contractNumber}`,
          config.koahost
        ).href,
              ...
              [
                {
                  datasets: [
                    {
                      data: keys.map((k) =>
                        grouped.get(k).reduce((o, e) => o + e.uniqueUsers, 0)
                      ),
                      label: "Unique Users",
                ...
                },
                {
                  datasets: [
                    {
                      data: keys.map((k) =>
                        grouped.get(k).reduce((o, e) => o + e.tokensConsumed, 0)
                      ),
                      label: keys,
                      backgroundColor: rchartColors,
                    },
                  ...
                  datasets: [
                    {
                      data: keys.map((k) =>
                        grouped
                          ...
                          .reduce(
                            (o, e) =>
                              o + Math.round(e.tokensConsumed / e.uniqueUsers),
                            0
                          )
                      ),
                      label: keys,
        ...
        .catch((err) => {
          ...
          console.error(
            `/api/aps/reports/summary/${contractNumber} error`,
            err
          );
        })
    ...
    toggleInfoWindow: function (marker, idx, item) {
      ...
      if (item !== this)
        this.$set(
          this.items,
          this.items.findIndex((e) => e.contractNumber === item.contractNumber),
          item
        );
    },
  },
