...
config.oauth2.authorizationURL =
  process.env.APS_AUTH_URL || config.oauth2.authorizationURL;
config.oauth2.callbackURL = process.env.APS_CALLBACK_URL || process.argv[4];
config.oauth2.clientID = process.env.APS_CLIENT_ID || process.argv[2];
config.oauth2.clientSecret = process.env.APS_CLIENT_SECRET || process.argv[3];
config.oauth2.tokenURL = process.env.APS_TOKEN_URL || config.oauth2.tokenURL;
config.scope = JSON.parse(process.env.APS_SCOPE || config.scope);
config.GoogleAPIKey = process.env.GoogleAPIKey || process.argv[5];
config.vuehost =
  process.env.VUE_HOST ||
  (process.env.NODE_ENV === "production" ? "origin" : config.vuehost);
...
config.logoutaccount_url =
  process.env.LOGOUTACCOUNT_URL || config.logoutaccount_url;
